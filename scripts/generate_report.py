...
def approx_score_from_severity(sev):
    return {
        "CRITICAL": 9.5, "HIGH": 7.5, "MEDIUM": 5.5, "LOW": 2.0, "UNKNOWN": 0.1, "INFO": 0.1
...
def draw_grouped_bars_by_severity(
    c, sem_counts, tri_counts, title, x, y, width=400, height=240
):
    top_title_h = 22
    bottom_axis_h = 22
    left_pad = 32
    ...
    ymax = max([sem_counts.get(s, 0) for s in severities] +
               [tri_counts.get(s, 0) for s in severities] + [0])
    if ymax < 5:
        ymax = 5
    ...
    for s in severities:
        ...
        v_sem = int(sem_counts.get(s, 0) or 0)
        v_tri = int(tri_counts.get(s, 0) or 0)
...
# =========================================================
# TÓPICOS (Semgrep, Trivy, Custom Review)
# =========================================================
...
def draw_custom_topics(c, findings):
    ...
    for f in findings:
        ...
        msg  = f"Mensagem: {f.get('message','')}" if f.get("message") else None
        snip = f"Snippet: {f.get('snippet','')[:180]}" if f.get("snippet") else None
        lines = [ln for ln in [fileline, msg, snip] if ln]
        y = draw_topic(c, y, head, lines, color=SEV_COLORS.get(sev, colors.grey))
    draw_footer(c); c.showPage()
...
def main():
    ...
    c.setFont("Helvetica-Bold", FONT_M)
    risks = {
        "Risco Imediato (CRITICAL)": semgrep_counts["CRITICAL"] + trivy_counts["CRITICAL"] + custom_counts["CRITICAL"],
        "Risco Alto (HIGH)":         semgrep_counts["HIGH"]     + trivy_counts["HIGH"]     + custom_counts["HIGH"],
        "Risco Médio (MEDIUM)":      semgrep_counts["MEDIUM"]   + trivy_counts["MEDIUM"]   + custom_counts["MEDIUM"],
    ...
    c.drawString(MARGIN_L, y, f"Totais → Semgrep: {sem_total} | Trivy: {tri_total} | Custom Review: {cus_total}")
    y -= 14
    ...
    # Barras Custom Review (severidades)
    draw_bars_with_values_single(
        c, custom_counts,
        "Severidades – Custom Security Review",
        MARGIN_L, y - 240, width=PAGE_W - MARGIN_L - MARGIN_R, height=240
    )
    draw_footer(c); c.showPage()
