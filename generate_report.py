...
COL_W_SEV   = 58
COL_W_ORIG  = 165
COL_W_DET   = CONTENT_W - COL_W_SEV - COL_W_ORIG - 8   # respiro entre colunas
...
def clamp_lines_to_fit(c, lines, width, max_lines, font="Helvetica", size=FONT_S):
    ...
    c.setFont(font, size)
    ell = "…"
    # Encorta último trecho até caber com “…”
...
def draw_table_header(c, y):
    ...
    c.drawString(MARGIN_L + COL_W_SEV + 6, y-4, "Origem")
    c.drawString(MARGIN_L + COL_W_SEV + COL_W_ORIG + 6, y-4, "Detalhe")
    c.setFillColor(colors.black)
    return y - 21
...
def draw_table_rows(c, items, start_y):
    ...
    for idx, it in enumerate(items):
        ...
        origem_txt  = it.get("file", it.get("pkg", ""))
        # detalhe: inclui CVSS no Trivy
        if "id" in it:
        ...
        # Quebras com limite por coluna
        sev_lines = [sev]  # badge ocupa 1 linha
        org_lines = wrap_lines(c, origem_txt, COL_W_ORIG - 12, size=FONT_S)
        det_lines = wrap_lines(c, detalhe_txt, COL_W_DET - 12, size=FONT_S)
        org_lines = clamp_lines_to_fit(c, org_lines, COL_W_ORIG - 12, MAX_LINES_ORIG, size=FONT_S)
        det_lines = clamp_lines_to_fit(c, det_lines, COL_W_DET - 12, MAX_LINES_DET,  size=FONT_S)
        ...
        # Badge de severidade (estreito e baixo)
        c.setFillColor(SEV_COLORS.get(sev, colors.black))
        c.roundRect(MARGIN_L + 6, y - ROW_PAD_Y - LINE_H, COL_W_SEV - 12, LINE_H+3, 2.5, fill=1, stroke=0)
        c.setFillColor(colors.white)
...
def main():
    ...
    draw_pie_with_values(c, semgrep_counts, "Distribuição por Severidade – Semgrep", MARGIN_L, y - 240)
    draw_bars_with_values(c, semgrep_counts, trivy_counts, "Semgrep × Trivy por Severidade", MARGIN_L, y - 500)
    draw_footer(c)
    c.showPage()
